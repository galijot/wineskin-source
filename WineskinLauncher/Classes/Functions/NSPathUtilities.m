...
+(NSString*)getMacPathForWindowsDrive:(char)driveLetter ofWrapper:(NSString*)file
{
    ...
    @autoreleasepool
    {
        ...
        path = [[NSFileManager defaultManager] destinationOfSymbolicLinkAtPath:drivePath];
...
+(NSString*)getMacPathForWindowsPath:(NSString*)exePath ofWrapper:(NSString*)file
{
    ...
    @autoreleasepool
    {
        ...
        if ([exePath hasPrefix:@"Z:"])
        {
            return [exePath substringFromIndex:2];
        }
        
        path = [self getMacPathForWindowsDrive:[exePath characterAtIndex:0] ofWrapper:file];
        
        path = [[NSString stringWithFormat:@"%@%@",path,[exePath substringFromIndex:2]]
...
+(NSString*)getFlagsOfWindowsPath:(NSString*)exePath
{
    ...
    // Considering that it start with ", that should work; first, split it with "
    NSString* result = @"";
    exePath = [exePath stringByReplacingOccurrencesOfString:@"\n" withString:@""];
...
{
    if ([structure isEqualToString:@""]) return TRUE;
    ...
    for (int index = 0; index < parts.count; index++)
    {
        ...
        {
            if (index == parts.count-1) return TRUE;
        }
        else
        {
            ...
            if (![path hasPrefix:part]) return FALSE;
            
            path = [path substringFromIndex:part.length];
        }
    }
